...
function SectionHeader({ number, title }) {
  return (
    <div className="flex items-center gap-3 px-5 py-4 bg-gradient-to-r from-[#1a2744] to-[#293C86] border-b-2 border-[#FF671F]">
      ...
      <h3 className="font-bold text-white tracking-wide text-sm sm:text-base">{title}</h3>
    </div>
  );
}
...
function UploadBox({ label, required, preview, inputRef, onChange, onClear, hint }) {
  return (
    <div className="flex flex-col items-center gap-3">
      ...
      >
        {preview ? (
          ...
          <div className="absolute inset-0 flex flex-col items-center justify-center gap-2 text-gray-400 group-hover:text-[#293C86] transition">
            ...
            <span className="text-[10px] font-semibold text-center px-2 leading-tight">Click to Upload</span>
          </div>
        )}
      </div>
      ...
      <p className="text-[10px] text-gray-400 text-center leading-relaxed">{hint}</p>
      <input
        ref={inputRef}
        type="file"
...
export default function MembershipFormPage() {
  ...
  const addChild = () => {
    if (children.length < 6) setChildren([...children, { name: "", education: "" }]);
  };
  const removeChild = (i) => {
  ...
  const handleSubmit = async (e) => {
    ...
    if (!photoFile || !aadharFile || !idCardFile) {
      setError("Please upload Passport Photo, Aadhar Card, and Martyr Soldier ID Card — all three are required.");
      return;
    }
    ...
    try {
      ...
      const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/api/membership`, {
        method: "POST",
        credentials: "include",
        body: fd,
      });
    ...
    return (
      <div className="min-h-screen bg-gray-50 flex items-center justify-center px-4">
        <div className="bg-white rounded-2xl shadow-xl p-10 max-w-md w-full text-center border border-gray-100">
          ...
          <h2 className="text-xl font-bold text-[#1a2744] mb-2">Application Submitted!</h2>
          <p className="text-sm text-gray-500 mb-6">
            Your Veer Nari / Veer Mata-Pita membership application has been received successfully. You will be contacted shortly.
          </p>
          <button
            onClick={resetForm}
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="bg-white border-b border-gray-200 px-4 sm:px-6 py-3">
        <div className="max-w-3xl mx-auto">
          <Link
            href="/"
            className="inline-flex items-center gap-2 text-sm font-bold text-[#293C86] hover:text-[#FF671F] transition-colors duration-200 group"
          >
            <svg
      ...
      <div className="relative bg-gradient-to-br from-[#0f1c3f] via-[#1a2744] to-[#293C86] py-12 px-4 text-center overflow-hidden">
        <div
          ...
          style={{ backgroundImage: "repeating-linear-gradient(-45deg,#fff 0px,#fff 1px,transparent 1px,transparent 18px)" }}
        />
        ...
        <form onSubmit={handleSubmit} noValidate className="space-y-6">
            ...
            <div className="p-5 sm:p-6 space-y-5">
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
                <Field label="Membership No." required>
                  <input name="membershipNo" value={form.membershipNo} onChange={handleChange}
                    type="text" className={inputCls} placeholder="e.g. MBR-2024-001" required />
                </Field>
              ...
              <Field label="Number of Martyr Soldiers in Family / Service Count" required>
                <input name="martyrCount" value={form.martyrCount} onChange={handleChange}
                  type="number" className={inputCls} placeholder="Enter count" min="1" required />
              </Field>
              <Field label="Full Name" required>
                <input name="fullName" value={form.fullName} onChange={handleChange}
                  type="text" className={inputCls} placeholder="Enter full name" required />
              </Field>
            </div>
          </div>
            ...
            <div className="p-5 sm:p-6 space-y-5">
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
                ...
                <Field label="Service Number" required>
                  <input name="serviceNumber" value={form.serviceNumber} onChange={handleChange}
                    type="text" className={inputCls} placeholder="e.g. IC-12345" required />
                </Field>
              </div>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
                <Field label="Date of Martyrdom / Service Date" required>
                  <input name="martyrdomDate" value={form.martyrdomDate} onChange={handleChange}
                    type="date" className={inputCls} required />
                </Field>
                <Field label="Place of Martyrdom" required>
                  <input name="placeOfMartyrdom" value={form.placeOfMartyrdom} onChange={handleChange}
                    type="text" className={inputCls} placeholder="Enter place" required />
                </Field>
              </div>
              <Field label="Information about Awards / Honors">
                <input name="awardsHonors" value={form.awardsHonors} onChange={handleChange}
                  type="text" className={inputCls} placeholder="e.g. Param Vir Chakra, Maha Vir Chakra…" />
              </Field>
              <Field label="Description of the Operation in which Martyrdom Occurred">
                <textarea name="operationDescription" value={form.operationDescription} onChange={handleChange}
                  className={`${inputCls} resize-y min-h-[90px]`} rows={4}
            ...
            <div className="p-5 sm:p-6 space-y-5">
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
                <Field label="Veer Nari Name (Widow Name)">
                  <input name="veerNariName" value={form.veerNariName} onChange={handleChange}
                    type="text" className={inputCls} placeholder="Full name of widow" />
                </Field>
                <Field label="Education">
                  <input name="veerNariEducation" value={form.veerNariEducation} onChange={handleChange}
                    type="text" className={inputCls} placeholder="e.g. B.A., 12th Pass…" />
                </Field>
              </div>
              ...
              <div className="space-y-3">
                <div className="flex items-center justify-between pb-2 border-b border-dashed border-gray-200">
                  <span className="text-[11px] font-bold tracking-wider uppercase text-[#1a2744]">Children Details</span>
                ...
                {children.map((child, i) => (
                  <div key={i} className="relative bg-gray-50 rounded-lg p-4 border border-gray-200">
                    <div className="flex items-center justify-between mb-3">
                      <span className="text-[11px] font-bold text-[#FF671F] tracking-wider uppercase">Child {i + 1}</span>
                      {children.length > 1 && (
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                      ...
                      <Field label="Education">
                        <input type="text" className={inputCls} placeholder="Class / Degree"
                          value={child.education} onChange={(e) => updateChild(i, "education", e.target.value)} />
                      </Field>
                    </div>
                  </div>
                ))}
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
                <Field label="Father's Full Name" required>
                  <input name="fatherName" value={form.fatherName} onChange={handleChange}
                    type="text" className={inputCls} placeholder="Father's full name" required />
                </Field>
                <Field label="Mother's Name" required>
                  <input name="motherName" value={form.motherName} onChange={handleChange}
                    type="text" className={inputCls} placeholder="Mother's full name" required />
                </Field>
              </div>
            </div>
          </div>
            ...
            <div className="p-5 sm:p-6 space-y-5">
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
                <Field label="Mobile Number 1" required>
                  <input name="mobile1" value={form.mobile1} onChange={handleChange}
                    type="tel" className={inputCls} placeholder="+91 XXXXX XXXXX" required />
                </Field>
                <Field label="Mobile Number 2">
                  <input name="mobile2" value={form.mobile2} onChange={handleChange}
                    type="tel" className={inputCls} placeholder="+91 XXXXX XXXXX" />
                </Field>
              </div>
              <Field label="Permanent Address" required>
                <textarea name="permanentAddress" value={form.permanentAddress} onChange={handleChange}
                  ...
                  placeholder="House No., Street, Village / Colony, Tehsil…" required />
              </Field>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
                <Field label="District" required>
                  <input name="district" value={form.district} onChange={handleChange}
                    type="text" className={inputCls} placeholder="Enter district" required />
                </Field>
          ...
          <div className="bg-[#FFF8F4] border border-[#FF671F]/30 rounded-xl p-5 sm:p-6 space-y-4">
            <div className="flex items-center gap-2 mb-1">
              ...
              <h4 className="text-sm font-bold text-[#1a2744] tracking-wide uppercase">Declaration / घोषणा</h4>
            </div>
            <div className="bg-white rounded-lg p-4 border border-[#FF671F]/20 space-y-3">
              <p className="text-sm text-gray-700 leading-relaxed italic">
                ...
                I will not violate the rules of the organization and will follow all the regulations."
              </p>
              <p className="text-sm text-gray-600 leading-relaxed">
                "मैं एतद्द्वारा घोषणा करता/करती हूँ कि मेरे द्वारा प्रदान की गई सभी जानकारी सत्य और सही है।
                मैं संगठन के नियमों का उल्लंघन नहीं करूँगा/करूँगी और सभी विनियमों का पालन करूँगा/करूँगी।"
              </p>
            </div>
            <label className="flex items-start gap-3 cursor-pointer group">
              ...
              <span className="text-sm text-gray-700 group-hover:text-gray-900 transition">
                I accept the above declaration and confirm that all information provided is <strong>true and correct</strong>.
              </span>
            </label>
          </div>
