...
export default function DonatePage() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const orderRes = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/api/donation/create-order`, {
        method: "POST",
      ...
      if (!orderRes.ok) {
        ...
        setErrors({ submit: `Server error (${orderRes.status}). Check your backend is running and NEXT_PUBLIC_API_URL is set correctly in .env.local` });
        setLoading(false);
        return;
      }
      ...
      const options = {
        ...
        handler: async (response) => {
          // Step 4: Verify payment + save to DB only on success
          try {
            const verifyRes = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/api/donation/verify-payment`, {
              method: "POST",
              headers: { "Content-Type": "application/json" },
              body: JSON.stringify({
              ...
              <div>
                  ...
                  {PRESET_AMOUNTS.map((amount) => (
                    ...
                    >
                      <div className={`w-4 h-4 rounded-full border-2 flex items-center justify-center flex-shrink-0 transition-colors ${
                        formData.selectedAmount === amount && !isOther ? "border-[#FF671F]" : "border-gray-400"
                      }`}>
                        {formData.selectedAmount === amount && !isOther && (
                      ...
                      <span className={`text-sm font-bold transition-colors ${
                        formData.selectedAmount === amount && !isOther ? "text-[#FF671F]" : "text-gray-700"
                      }`}>
                ...
                <label
                  ...
                  className={`flex items-center gap-3 px-4 py-3.5 rounded-xl border-2 cursor-pointer transition-all duration-200 select-none mb-3 ${
                    isOther ? "border-[#FF671F] bg-orange-50" : "border-gray-200 bg-gray-50 hover:border-gray-300 hover:bg-gray-100"
                  }`}
                >
                  <div className={`w-4 h-4 rounded-full border-2 flex items-center justify-center flex-shrink-0 transition-colors ${
                  ...
                  <span className={`text-sm font-bold transition-colors ${isOther ? "text-[#FF671F]" : "text-gray-700"}`}>
                    Other Amount
                  </span>
                </label>
